namespace Dixin.Linq.Lambda
{
    ...
    public static class ChurchAggregateList
    {
        // IsNull = node => node(value => _ => False)(True)
        public static Boolean IsNull<T>(this AggregateListNode<T> node) => (Boolean)node(value => _ => False)(True);

        public static T Value<T>(this AggregateListNode<T> node) => (T)node(_ => value => value)(Functions<T>.Id);
        ...
        public static AggregateListNode<T> Next<T>(this AggregateListNode<T> node) =>
            ((Tuple<AggregateListNode<T>, AggregateListNode<T>>)node
                (tuple => value => ((Tuple<AggregateListNode<T>, AggregateListNode<T>>)tuple).Shift(ChurchAggregateList<T>.Create(value)))
                (ChurchTuple<AggregateListNode<T>, AggregateListNode<T>>.Create(ChurchAggregateList<T>.Null)(ChurchAggregateList<T>.Null)))
            .Item1();

        // Index = start => index => index(Next)(start)
        public static AggregateListNode<T> Index<T>(this AggregateListNode<T> start, Numeral index) => (AggregateListNode<T>)index(node => ((AggregateListNode<T>)node).Next())(start);
    }
}
