...
{
    internal static void MergeRarbgTV(string directory, string originalVideoSearchPattern = "", string originalSubtitleSearchPattern = "", Func<string, string>? getDubbedVideo = null, Func<string, string>? renameSubtitle = null, bool? isTV = null)
    {
        string[] originalVideos = originalVideoSearchPattern.IsNotNullOrWhiteSpace()
            ...
            : Directory
                ...
                .ToArray();
        originalVideos.ForEach(originalVideo => MergeDubbed(originalVideo, getDubbedVideo is not null ? getDubbedVideo(originalVideo) : string.Empty, isTV: isTV));

        string[] originalSubTiles = originalSubtitleSearchPattern.IsNotNullOrWhiteSpace()
            ? Directory
                .GetFiles(directory, originalSubtitleSearchPattern, SearchOption.AllDirectories)
            : Directory
                .EnumerateFiles(directory, PathHelper.AllSearchPattern, SearchOption.AllDirectories)
                .Where(file => file.IsSubtitle() && !Path.GetFileNameWithoutExtension(file).ContainsIgnoreCase(".DUBBED."))
                .ToArray();
        originalSubTiles.ForEach(subtitle =>
        {
            string renamedSubtitle = renameSubtitle is not null 
                ? renameSubtitle(subtitle) 
                : subtitle.ContainsIgnoreCase(".chs.") ? subtitle.Replace(".chs.", ".2Audio.chs.") : PathHelper.AddFilePrefix(subtitle, ".2Audio");
            FileHelper.Move(subtitle, renamedSubtitle);
        });
    }

    internal static void Encode(string input, string output = "", bool overwrite = false, bool? estimateCrop = null, string? relativePath = null, int retryCount = 10)
    {
    ...
    internal static void MergeDubbed(string input, string dubbed = "", string output = "", bool? isTV = null)
    {
        if (!isTV.HasValue)
        {
        ...
        {
            VideoFileInfo inputVideo = VideoFileInfo.Parse(input);
            if (dubbed.IsNullOrWhiteSpace())
            {
                dubbed = (inputVideo with { Edition = "DUBBED" }).Name;
            }
        ...
        ProcessHelper.StartAndWait(
            "ffmpeg",
            $"""
                -i "{ input}" -i "{ dubbed}" -c copy -map_metadata 0 -map 0 -map 1:a "{ output}"
                """,
            window: true);
    }
