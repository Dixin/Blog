    ...
    string Extension) : IVideoFileInfo, ISimpleParsable<VideoMovieFileInfo>, IDefaultSettings
{
    ...
    private static Regex NameRegex => nameRegex ??= RegexHelper.Create(
        ...
        @$"(\{Video.VersionSeparator}({Settings.TopEnglishKeyword}|{Settings.TopForeignKeyword}|{Settings.PreferredOldKeyword}|\[{Settings.PreferredNewKeyword}\.(AG|AM|BZ|LT|ME|MX)\]|[a-zA-Z0-9@]+?))?",
        @"(\.[0-9]+Audio)?",
        @"(\.watermark)?",
