...
internal static partial class Video
{
        ...
        if (skipTopPreferred)
        {
            parallelFiles = parallelFiles
                .Where(video =>
                {
                    ...
                    return !name.ContainsIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") && !name.ContainsIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}") && !name.ContainsIgnoreCase(settings.KeywordPreferredNew) && !name.ContainsIgnoreCase(settings.KeywordPreferredOld);
                });
        }
    ...
    internal static async Task PrintLibraryDuplicateImdbIdAsync(ISettings settings, Action<string>? log = null, CancellationToken cancellationToken = default)
    {
        ...
        existingMetadata
            ...
            .ForEach(pair => pair
                .Value
                .Keys
                .Select(video => Path.Combine(settings.DirectoryLibrary, video))
                .Order()
                .Append(string.Empty)
                .Prepend(pair.Key)
                .ForEach(log));
    ...
    internal static void PrintDuplicateImdbId(Action<string>? log = null, params string[] drives)
    {
        log ??= Logger.WriteLine;
        ...
        drives
            .AsParallel()
            .SelectMany(drive => Directory.EnumerateFiles(drive, ImdbMetadataSearchPattern, new EnumerationOptions() { IgnoreInaccessible = true, RecurseSubdirectories = true }))
            .Where(metadata => !metadata.ContainsIgnoreCase($"{Path.DirectorySeparatorChar}Delete"))
            .Do(metadata =>
            {
    ...
    internal static async Task PrintMovieVersions(ISettings settings, Action<string>? log = null, CancellationToken cancellationToken = default, params (string Directory, int Level)[] directories)
    {
        ...
        ConcurrentDictionary<string, List<PreferredFileMetadata>> preferredFiles = await settings.LoadMetadataPreferredMoviesFilesAsync(cancellationToken);
        ...
        HashSet<string> ignore = await settings.LoadMetadataAllMoviesIgnoredAsync(cancellationToken);
        ...
        directories
            ...
            .SelectMany(directory => EnumerateDirectories(directory.Directory, directory.Level)
                .Where(movie => !movie.ContainsIgnoreCase($"{Path.DirectorySeparatorChar}Delete"))
                .Select(movie =>
                {
                    string[] files = Directory.GetFiles(movie);
                    return (movie, files, ImdbId: ImdbMetadata.TryGet(files, out string? _, out string? imdbId) ? imdbId : string.Empty);
                })
                .Where(movie => movie.ImdbId.IsNotNullOrWhiteSpace()))
            .OrderBy(movie => movie.movie)
            .AsSequential()
            .ForEach(movieInfo =>
            {
                string imdbId = movieInfo.ImdbId;
                string movie = movieInfo.movie;
                (string File, string Name, VideoMovieFileInfo Info)[] videos = movieInfo.files
                ...
                (string File, string Name, VideoMovieFileInfo Info)[] preferredOrTopVideos = videos.Where(video => video.Name.ContainsIgnoreCase("-VXT") || video.Name.ContainsIgnoreCase("-RARBG") || video.Name.ContainsIgnoreCase("YTS") || video.Name.ContainsIgnoreCase("YIFY"))
                    .ToArray();
                if (preferredOrTopVideos.Any())
                {
                    (string File, string Name, VideoMovieFileInfo Info)[] bluRayPreferredOrTopVideos = preferredOrTopVideos.Where(video => video.Name.ContainsIgnoreCase("BluRay")).ToArray();
                    if (!bluRayPreferredOrTopVideos.Any())
                    {
                        (PreferredMetadata Metadata, KeyValuePair<string, string> Version)[] bluRayPreferred = availablePreferredMetadata
                            .SelectMany(metadata => metadata.Availabilities, (metadata, version) => (metadata, version))
                            ...
                            .ToArray();
                        if (bluRayPreferred.Any(preferred => preferred.Version.Key.ContainsIgnoreCase("1080p")))
                        {
                            bluRayPreferred = bluRayPreferred
                                .Where(preferred => preferred.Version.Key.ContainsIgnoreCase("1080p"))
                                .ToArray();
                        }
                        if (bluRayPreferred.Any())
                        {
                    ...
                    else if (!bluRayPreferredOrTopVideos.Any(video => video.Info.GetDefinitionType() is DefinitionType.P1080))
                    {
                        (PreferredMetadata Metadata, KeyValuePair<string, string> Version)[] bluRayPreferred = availablePreferredMetadata
                            .SelectMany(metadata => metadata.Availabilities, (metadata, version) => (metadata, version))
                            .Where(metadataVersion => metadataVersion.version.Key.ContainsIgnoreCase("BluRay") && metadataVersion.version.Key.ContainsIgnoreCase("1080p"))
                            .ToArray();
                        if (bluRayPreferred.Any())
                        {
                            bluRayPreferred.ForEach(preferredMetadata =>
    ...
    internal static async Task PrintLibraryMovieVersions(ISettings settings, Action<string>? log = null, CancellationToken cancellationToken = default, params string[] drives)
    {
        log ??= Logger.WriteLine;
        Dictionary<string, TopMetadata[]> x265Metadata = await settings.LoadMetadataTopMoviesX265Async(cancellationToken);
        Dictionary<string, TopMetadata[]> h264Metadata = await settings.LoadMetadataTopMoviesH264Async(cancellationToken);
        ...
        ConcurrentDictionary<string, List<PreferredMetadata>> preferredMetadata = await settings.LoadMetadataPreferredMoviesAsync(cancellationToken);
        HashSet<string> ignore = await settings.LoadMetadataAllMoviesIgnoredAsync(cancellationToken);
        ConcurrentDictionary<string, ConcurrentDictionary<string, VideoMetadata>> library = await settings.LoadMetadataLibraryMoviesAsync(cancellationToken);
        ...
        library.Values.ForEach(dic => dic.Keys
            ...
            .ForEach(file => dic.Remove(file, out _)));
        library.Keys.Where(imdbId => library[imdbId].IsEmpty() || imdbIds.Contains(imdbId))
            .ForEach(imdbId => library.Remove(imdbId, out _));
        ;
        library
            .Keys
            ...
            .ForEach(imdbId =>
            {
                (string File, string Name, VideoMovieFileInfo Info)[] videos = library[imdbId].Keys
                    .Where(file => file.IsVideo())
                    .Select(file => (file, PathHelper.GetFileNameWithoutExtension(file), VideoMovieFileInfo.Parse(file)))
                    .ToArray();
                (string File, string Name, VideoMovieFileInfo Info)[] preferredVideos = videos
    ...
    {
        log ??= Logger.WriteLine;
        Dictionary<string, TopMetadata[]> x265Metadata = await settings.LoadMetadataTopTVX265Async(cancellationToken);
    ...
    {
        log ??= Logger.WriteLine;
        string[] specialImdbIds = await settings.LoadMetadataAllMoviesSpecialAsync(cancellationToken);
        Dictionary<string, TopMetadata[]> x265Summaries = await settings.LoadMetadataTopMoviesX265Async(cancellationToken);
        Dictionary<string, TopMetadata[]> h264Summaries = await settings.LoadMetadataTopMoviesH264Async(cancellationToken);
    ...
    internal static async Task PrintMovieImdbIdErrorsAsync(ISettings settings, bool ignoreJsonImdbId = false, Action<string>? log = null, CancellationToken cancellationToken = default, params (string Directory, int Level)[] directories)
    {
        ...
        HashSet<string> topDuplications = settings.MetadataTopMoviesDuplication.ToHashSetOrdinalIgnoreCase();
        ...
        ILookup<string, string> preferredTitlesToImdbIds = preferredFileMetadata
            ...
            .ToLookup(
                metadata => metadata.File
                    .ReplaceIgnoreCase($".{settings.KeywordPreferredOld}", $"{VersionSeparator}{settings.KeywordPreferredOld}")
        ...
        params string[] drives)
    {
        ...
        ILookup<string, string> topMagnetUris = (await File.ReadAllLinesAsync(settings.MetadataTopMediaMagnetUrls, cancellationToken))
        ...
        await imdbIds
            ...
            .ForEachAsync(async (imdbMetadata, index, token) =>
            {
                ...
                if (x265Metadata.TryGetValue(imdbMetadata.ImdbId, out TopMetadata[]? x265Videos))
                {
                    ...
                    {
                        if (x265Videos.Any(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")) && x265Videos.Any(video => !(video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}"))))
                        {
                            excluded.AddRange(x265Videos.Where(video => !(video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}"))));
                            x265Videos = x265Videos.Where(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")).ToArray();
                        }
                    ...
                    if (x265Videos.Any(video => !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") && !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")))
                    {
                        excluded.AddRange(x265Videos.Where(video => !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") && !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")));
                        x265Videos = x265Videos.Where(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")).ToArray();
                    }
                ...
                if (h264Metadata.TryGetValue(imdbMetadata.ImdbId, out TopMetadata[]? h264Videos))
                {
                    ...
                    {
                        if (h264Videos.Any(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")) && h264Videos.Any(video => !(video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}"))))
                        {
                            excluded.AddRange(h264Videos.Where(video => !(video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}"))));
                            h264Videos = h264Videos.Where(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")).ToArray();
                        }
                    ...
                    if (h264Videos.Any(video => !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") && !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")))
                    {
                        excluded.AddRange(h264Videos.Where(video => !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") && !video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")));
                        h264Videos = h264Videos.Where(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")).ToArray();
                    }
                    ...
                    await preferredVideos
                        ...
                        .ForEachAsync(async preferredMetadata =>
                        {
                            ...
                            preferredMetadata.PreferredAvailabilities
                                .Select(availability => Path.Combine(settings.DirectoryMetadataAllMoviesCache, $"{preferredMetadata.ImdbId}.{availability.Value.GetPreferredExtractTopic()}{TorrentHelper.TorrentExtension}"))
                                .Do(log)
                                .ForEach(file =>
                                {
                                    if (File.Exists(file))
        ...
        CancellationToken cancellationToken = default)
    {
        log ??= Logger.WriteLine;
        Dictionary<string, TopMetadata[]> x265Metadata = await settings.LoadMetadataTopTVX265Async(cancellationToken);
        Dictionary<string, string> metadataFiles = Directory.EnumerateFiles(settings.DirectoryMetadataAllTV).ToDictionary(file => ImdbMetadata.TryGet(file, out string? imdbId) ? imdbId : string.Empty);
        ...
        if (updateMetadata)
        {
            await imdbIds.ForEachAsync(async (ImdbMetadata imdbMetadata, int index) =>
            {
                ...
                {
                    await Retry.FixedIntervalAsync(
                        async () => await DownloadImdbMetadataAsync(imdbMetadata.ImdbId, settings.DirectoryMetadataAllTV, settings.DirectoryMetadataAllTVCache, metadataFiles.Values.ToArray(), cacheFiles, playwrightWrapper, overwrite: true, useCache: false, log: log, cancellationToken: cancellationToken),
            ...
            .ForEachAsync(
                async (imdbMetadata, index, token) =>
                {
                    if (x265Metadata.TryGetValue(imdbMetadata.ImdbId, out TopMetadata[]? h265Videos))
                    {
                        ...
                        {
                            if (h265Videos.Any(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")) && h265Videos.Any(video => !(video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}"))))
                            {
                                excluded.AddRange(h265Videos.Where(video => !(video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}"))));
                                h265Videos = h265Videos.Where(video => video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}") || video.Title.EndsWithIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")).ToArray();
                            }
                        ...
                        await h265Videos.ForEachAsync(async metadata =>
                        {
                            ...
                            string file = Path.Combine(settings.DirectoryMetadataAllTVCache, $"{metadata.ImdbId}-{metadata.Title}{ImdbCacheExtension}");
                            string html;
    ...
    internal static async Task PrintTVContrast(ISettings settings, Action<string>? log = null, CancellationToken cancellationToken = default)
    {
        ...
        ILookup<string, ContrastMetadata> contrastMetadata = (await JsonHelper.DeserializeFromFileAsync<ContrastMetadata[]>(settings.MetadataContrastTV, cancellationToken))
            .Where(metadata => metadata.ImdbId.IsNotNullOrWhiteSpace()
                && metadata.Title.ContainsIgnoreCase($"{VersionSeparator}{settings.KeywordContrast}")
                ...
                Seasons: Directory
                    ...
                    .Select(season => (
                        ...
                        Episodes: Directory.EnumerateFiles(season).Where(file =>
                        {
                            ...
                            return file.IsVideo()
                                && !name.ContainsIgnoreCase($"{VersionSeparator}{settings.KeywordTopEnglish}")
                                && !name.ContainsIgnoreCase($"{VersionSeparator}{settings.KeywordTopForeign}")
                                && !name.ContainsIgnoreCase($"{VersionSeparator}{settings.KeywordContrast}");
                        }).ToArray()))
                    .ToArray()))
    ...
    internal static async Task<List<string>> PrintImdbFranchiseAsync(ISettings settings, string directory, int level = DefaultDirectoryLevel, Action<string>? log = null, CancellationToken cancellationToken = default)
    {
        ...
        ConcurrentDictionary<string, List<string>> libraryMetadataFiles = await LoadLibraryMovieMetadataFilesAsync(settings, cancellationToken);
            ...
            .ForEach(movie =>
            {
                    ...
                    && followedBy.Any())
                {
                    ...
                    followedBy.ForEach(array =>
                    {
                        ...
                        if (libraryMetadataFiles.TryGetValue(imdbId, out List<string>? localMetadataFiles))
                        {
                            isFranchise = true;
    ...
    internal static async Task PrintPopularMoviesAsync(ISettings settings, Action<string>? log = null, CancellationToken cancellationToken = default)
    {
        log ??= Logger.WriteLine;

        ConcurrentDictionary<string, ImdbMetadata> mergedMetadata = await settings.LoadMetadataAllMoviesAsync(cancellationToken);
        ParallelQuery<(int Order, string Title, string Category, string Id, string MagnetUrl, string ImdbId, DateTime DateTime)> topMedia = await settings.LoadMetadataTopMediaAsync(cancellationToken);
        ConcurrentDictionary<string, List<PreferredMetadata>> preferredMovies = await settings.LoadMetadataPreferredMoviesAsync(cancellationToken);

        ImdbMetadata[] movies = mergedMetadata
            .Values
            .Where(metadata => metadata.AggregateRating is { RatingCount: >= 50_000 } && string.CompareOrdinal(metadata.AggregateRating.RatingValue, "7.5") >= 0)
            .ToArray();
        movies.ForEach(metadata =>
        {
            log($"{metadata.ImdbId} {metadata.Title.EscapeMarkup()} {metadata.Year} {metadata.FormattedAggregateRating} {metadata.FormattedAggregateRatingCount} {metadata.FormattedContentRating}");

            (int Order, string Title, string Category, string Id, string MagnetUrl, string ImdbId, DateTime DateTime) firstOrDefault = topMedia.FirstOrDefault(media => media.ImdbId.EqualsIgnoreCase(metadata.ImdbId) && media.Title.ContainsIgnoreCase("1080p") && (media.Title.EndsWithIgnoreCase("-RARBG") || media.Title.EndsWithIgnoreCase("-VXT")));
            if (firstOrDefault.ImdbId.IsNotNullOrWhiteSpace())
            {
                log($"[green]{firstOrDefault.Title}[/]");
                log(firstOrDefault.MagnetUrl);
            }
            else
            {
                preferredMovies[metadata.ImdbId].ForEach(movie => movie.PreferredAvailabilities.ForEach(availability =>
                {
                    log($"[yellow]{movie.Title.EscapeMarkup()} {movie.Year} {availability.Key}[/]");
                    log($"magnet:?xt=urn:btih:{availability.Value.Split("/", StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Last()}");
                }));
            }
