...
public static class FfmpegHelper
{
        ...
        int retryCount = Video.IODefaultRetryCount, Action<string>? log = null, CancellationToken cancellationToken = default, params TimeSpan[] cropTimestamps)
    {
        ...
        string audio = videoMetadata.AudioBitRates.All(bitRate => bitRate > 260_000) && !Video.IsAtmos(input) ? "aac -ar 48000 -b:a 256k -ac 6" : "copy";
        ...
        int? maxDegreeOfParallelism = null, int? cropTimestampCount = DefaultTimestampCount, bool sample = false, Action<string>? log = null, CancellationToken cancellationToken = default)
    {
        ...
        await videos.ParallelForEachAsync(
                async (input, index, token) =>
                {
                    ...
                    {
                        log(exception.ToString().EscapeMarkup());
                    }
                },
        ...
        inputVideos.ForEach(video => log(video.EscapeMarkup()));

        log($"Skipped {skippedInputVideos.Count} input videos:");
        skippedInputVideos.ForEach(video => log(video.EscapeMarkup()));
    ...
    internal static async Task ExtractAllAsync(ISettings settings, string inputDirectory, Func<string, bool>? predicate = null, bool isTV = false, bool skipParsing = false, bool deleteInput = false, bool isSubtitleOnly = false, bool isDryRun = false, Action<string>? log = null, CancellationToken cancellationToken = default, params Func<string, string>[] outputVideos)
    {
        log ??= Logger.WriteLine;
