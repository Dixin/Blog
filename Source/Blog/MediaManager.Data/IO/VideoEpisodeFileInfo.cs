    ...
    string Extension) : IVideoFileInfo, ISimpleParsable<VideoEpisodeFileInfo>, IDefaultSettings
{
    ...
    private static Regex NameRegex => nameRegex ??= RegexHelper.Create(
        ...
        @$"(\{Video.VersionSeparator}({Settings.KeywordTopEnglish}|{Settings.KeywordTopForeign}|{Settings.KeywordPreferredOld}|\[{Settings.KeywordPreferredNew}\.(AG|AM|LT|ME|MX)\]|[a-zA-Z0-9@]+?))?",
        @"(\.[0-9]+Audio)?",
        @"(\.watermark)?",
